...
gtts-cli -f "$NAUTILUS_SCRIPT_SELECTED_FILE_PATHS" -o "${NAUTILUS_SCRIPT_SELECTED_FILE_PATHS%.*}_PRE.mp3" -l 'es-us'
ffmpeg -i "${NAUTILUS_SCRIPT_SELECTED_FILE_PATHS%.*}_PRE.mp3" -af "atempo=2,asetrate=r=22K,volume=5" -vn "${NAUTILUS_SCRIPT_SELECTED_FILE_PATHS%.*}.mp3" -y
rm "${NAUTILUS_SCRIPT_SELECTED_FILE_PATHS%.*}_PRE.mp3"
